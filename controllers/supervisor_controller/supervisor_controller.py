...
agent0_waypoints = np.array(
    [[-10.16, 10.00], [-10.16, 5.0], [-10.16, 0.0], [-5.0, 5.0]]
)  # [[x1,y1],[x2,y2],...]
agent1_waypoints = np.array(
    [[-5.5, 10.15], [-11.92, 10.15], [-11.92, 3.63], [-15.26, 7.92]]
)
agent2_waypoints = np.array(
    [[-0.65, 10.1], [-0.65, 3.21], [3.58, -2.58], [-2.43, -8.48]]
)
...
agent_init_pos_list = [
    agent0_waypoints[0].tolist(),
    agent1_waypoints[0].tolist(),
    agent2_waypoints[0].tolist(),
    agent3_waypoints[0].tolist(),
]
...
agent_receiver_list = [
    agent0_state_receiver,
    agent1_state_receiver,
    agent2_state_receiver,
    agent3_state_receiver,
]
...
agent_emitter_list = [
    control_agent0_emitter,
    control_agent1_emitter,
    control_agent2_emitter,
    control_agent3_emitter,
]

# Place agents at initial positions
for idx, agent_init_pos in enumerate(agent_init_pos_list):
    agent_translation_field = supervisor_robot.getFromDef("agent_" + str(idx)).getField(
        "translation"
    )
...
for i in range(len(agent_list)):
    ...
    agent_x_ref_list.append(
        agent_MPC_list[i].waypoints_to_x_ref(
            agent_waypoint_list[i],
            interpolated_dist=0.25,
...
while supervisor_robot.step(timestep) != -1:
    ...
    for i in range(len(agent_list)):
        agent_x_local_ref_list[i], agent_target_idx_list[i] = agent_MPC_list[
            i
